...
describe('Cypress Demo Website Automation Scenarios', () => {
  ...
  it('Check if User can modify profile successfully', () => {
    ...
    // Check if bio is set
    cy.get('p[ng-bind="::$ctrl.profile.bio"]').should('have.text', profileShortBio)

    // Check if Profile Picture is set
    cy.get('.user-img')
      .invoke('attr', 'src')
      .should('eq', profilePictureUrl)
  ...
  it('Check if display of mocked articles is correct', () => {
    ...
    cy.fixture('articles').then((articlesData) => {
      ...
      // Check first article
      cy.get('article-preview').eq(0).within(() => {
      ...
      cy.get('article-preview').eq(1).within(() => {
